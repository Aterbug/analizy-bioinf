...
paste("średnia:", sr, "mediana:", md, "wariancja:", wr)
```
...
```{r}
elementy <- seq(from = 1, to = 100, by = 1)
proba <- sample(elementy, size = 50, replace = FALSE)  # Losowe 3 różne elementy
print(proba)
min(proba)
max(proba)
sum(proba)
...
dane_ramka <- data.frame(
  nazwa = c("baton", "baton2", "baton3"),
  ilość = c(28, 34, 23),
  cena = c(1.55,2.35,0.95)
)
dane_ramka
```
...
```{r}
dane_ramka$wartość <- c(dane_ramka$cena*dane_ramka$ilość)
dane_ramka
```
...
```{r}
wek<-c(-3,2,5)
ifelse(wek > 0,   "Dodatni","Niedodatni")
